...
from NetandLoss_keysPtsDeformation_utils.cages import deform_with_MVC
from NetandLoss_keysPtsDeformation_utils.networks import Linear, MLPDeformer2, PointNetfeat
from NetandLoss_keysPtsDeformation_utils.utils import normalize_to_box, sample_farthest_points


class KeysDeform2Keys(nn.Module):
    @staticmethod
    ...
    def __init__(self, opt):
        super(KeysDeform2Keys, self).__init__()
        
        self.opt = opt
        
        template_vertices, template_faces = self.create_cage()  # 新建单位求笼，输出顶点和面
        self.init_template(template_vertices, template_faces)
        self.init_networks(256, 3, opt)
        self.init_optimizer()


    def create_cage(self):
        # cage (1, N, 3)
        mesh = pytorch3d.utils.ico_sphere(self.opt.ico_sphere_div, device='cuda:0')  # 为单位球创建顶点和面，第一个参数是迭代次数，第二个是设备
    ...
    def init_optimizer(self):
        ...
        self.optimizer.add_param_group({'params': self.influence_param, 'lr': 10 * self.opt.lr})  # 好像是又加了一些参数进来
...
if __name__ == "__main__":
    # 网络初始化
    kD = KeysDeform2Keys()
    source_shape = torch.rand([1, 1024, 3]).cuda()
    target_shape = torch.rand([1, 1024, 3]).cuda()
    # 前向
    outputs = kD(source_shape, target_shape)
    #计算loss