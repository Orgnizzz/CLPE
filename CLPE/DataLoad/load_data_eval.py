...
from DataLoad.eval_utils_v1 import get_3d_bbox, transform_coordinates_3d, compute_3d_iou_new
...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='test',
        ...
        KeyPoints_file_path = ['KeyPoints/CategoryKeyPoints.pkl']
        self.insKeyPoints_file_path = ['KeyPoints/camera_train.pkl', 'KeyPoints/real_train.pkl',
                               'KeyPoints/camera_val.pkl', 'KeyPoints/real_test.pkl', ]

        if mode == 'train':
            del img_list_path[2:]
            del model_file_path[2:]
            del self.insKeyPoints_file_path[2:]
        else:
            del img_list_path[:2]
            del model_file_path[:2]
            del self.insKeyPoints_file_path[:2]
        if source == 'CAMERA':
            ...
            del self.insKeyPoints_file_path[-1]
        elif source == 'Real':  # 训练实拍数据用的是real
            del img_list_path[0]
            del model_file_path[0]
            del self.insKeyPoints_file_path[0]
        else:
        ...
        self.keyRootDir = os.path.abspath(os.path.join(data_dir, os.path.pardir)) + '/'
        self.keyPoints = {}
        for path in KeyPoints_file_path:
            with open(self.keyRootDir + path, 'rb') as f:
                self.keyPoints.update(cPickle.load(f))

        self.keysAll = {}  # 读取与模型关联的所有类和实例的关键点
        with open(self.keyRootDir + self.insKeyPoints_file_path[0], 'rb') as f:
            self.keysAll.update(cPickle.load(f))
    ...
    def __getitem__(self, index):
        ...
        rgb = io.imread(img_path + '_color.png')
        ...
        if os.path.exists(depth_path):
            ...
            return None
        num_instance = len(detection_dict['pred_class_ids'])  # 图片中物体类别的检测结果和个数
        ...
        for j in range(num_instance):  # 遍历每个图片中的每个物体
            ...
            # 读取每个类的关键点
            # print('11111222222221111111')
            # print('cat_id : ', cat_id)
            # print('len(self.keyPoints)', len(self.keyPoints))
            # print('self.cat_names : ', self.cat_names)
            # print('cat_names[cat_id]', self.cat_names[cat_id])
            # print(self.keyPoints[self.cat_names[cat_id]])


            pic_labelPath = img_path + '_label.pkl'
            picInform = {}
            with open(pic_labelPath, 'rb') as f:
                picInform.update(cPickle.load(f))
            modelNameinPic = picInform['model_list']  # 根据图片的名字，获取图片中包含的模型的名字
            # 遍历模型名字，读取类关键点和实例关键点
            for model in modelNameinPic:
                # print(modelNameinPic, model)
                # print(modelNameinPic.index(model))
                # print(modelNameinPic.index(model).shape)
                # asdf
                insKeys[model] = torch.as_tensor(self.keysAll[model+'_target_keypoints'], dtype=torch.float32).contiguous()
                # insKeys.append(self.keysAll[model+'_target_keypoints'])
            catKeys.append(self.keyPoints[self.cat_names[cat_id-1]].astype(np.float32))
        ...
        data_dict = {}
        data_dict['roi_img'] = torch.as_tensor(roi_imgs.astype(np.float32)).contiguous()  # 裁剪的图片
        ...
        #类和实例关键点
        data_dict['catKeys'] = torch.as_tensor(np.array(catKeys), dtype=torch.float32).contiguous()
        data_dict['insKeys'] = insKeys
        # print(data_dict['cat_id'])
        # print(data_dict['catKeys'])
        # # 输出真实的尺寸，输出模型用
        # gtSone = {}
        # for insIdx in gts['instance_ids']:
        #     gtSone[str(insIdx)] = torch.as_tensor(gts['scales'][insIdx-1], dtype=torch.float32).contiguous()  # 物体编号对应的尺寸
        # data_dict['gtSone'] = gtSone
        # roi_img和roi_depth 都是分割方块中的物体、roi_mask_deform是方块中的mask， 据此，获得对应的点云和图片的choose
        # print(data_dict['roi_mask'].shape, data_dict['roi_depth'].shape, data_dict['cam_K'].shape, data_dict['roi_coord_2d'].shape)

        PC = torch.zeros(data_dict['roi_mask'].shape[0], 1028, 3)
        choose = torch.zeros(data_dict['roi_mask'].shape[0], 1028)
        choose = torch.tensor(choose, dtype=torch.int64)
        for numBs in range(data_dict['roi_mask'].shape[0]):
            PC[numBs], choose[numBs] = PC_sample(data_dict['roi_mask'][numBs], data_dict['roi_depth'][numBs], data_dict['cam_K'][numBs],
                                   data_dict['roi_coord_2d'][numBs])
